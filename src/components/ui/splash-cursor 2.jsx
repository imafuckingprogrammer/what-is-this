...
function SplashCursor({
    // Fun effects settings - enhanced for more visual impact!
    SIM_RESOLUTION = 128, // Increased for smoother effects
    DYE_RESOLUTION = 1024, // Higher resolution for crisp colors
    CAPTURE_RESOLUTION = 512, // Better capture quality
    DENSITY_DISSIPATION = 1.5, // Slower dissipation for longer trails
    VELOCITY_DISSIPATION = 0.8, // Much slower velocity decay for flowing effects
    PRESSURE = 0.12, // Higher pressure for more dynamic movement
    PRESSURE_ITERATIONS = 25, // More iterations for better fluid behavior
    CURL = 8.0, // Much higher curl for swirly, turbulent effects
    SPLAT_RADIUS = 0.25, // Larger splat radius for bigger impact
    SPLAT_FORCE = 8000, // Higher force for more dramatic effects
    SHADING = true, // Enable shading for depth
    COLOR_UPDATE_SPEED = 15, // Faster color changes for rainbow effects
    BACK_COLOR = { r: 0.02, g: 0.02, b: 0.05 }, // Subtle dark blue background
    TRANSPARENT = true,
    className = "",
    // New fun effect parameters
    RAINBOW_MODE = true, // Enable rainbow color cycling
    PARTICLE_TRAILS = true, // Enable particle trail effects
    GLOW_INTENSITY = 2.0, // Glow effect intensity
    TURBULENCE_SCALE = 3.0, // Extra turbulence for chaotic effects
}) {
    ...
    useEffect(() => {
        // Prevent multiple instances - singleton pattern with unique ID
        const instanceId = Math.random().toString(36).substr(2, 9);

        if (window.splashCursorActive && window.splashCursorInstance) {
            console.log('Splash cursor already active, skipping initialization');
            return;
        }

        window.splashCursorActive = true;
        window.splashCursorInstance = instanceId;
            ...
            const initializeSplashCursor = () => {
                ...
                try {
                    ...
                    const adjustedConfig = {
                        SIM_RESOLUTION: isLowPerformance ? 32 : SIM_RESOLUTION,
                        DYE_RESOLUTION: isLowPerformance ? 256 : DYE_RESOLUTION,
                        CAPTURE_RESOLUTION: isLowPerformance ? 128 : CAPTURE_RESOLUTION,
                        DENSITY_DISSIPATION,
                        VELOCITY_DISSIPATION,
                        PRESSURE,
                        PRESSURE_ITERATIONS: isLowPerformance ? 10 : PRESSURE_ITERATIONS,
                        CURL,
                        SPLAT_RADIUS,
                        SPLAT_FORCE: isLowPerformance ? 3000 : SPLAT_FORCE,
                        SHADING: false, // Always disabled for performance
                        COLOR_UPDATE_SPEED,
                        PAUSED: false,
                        BACK_COLOR,
                        TRANSPARENT,
                    ...
                    if (!ext.supportLinearFiltering) {
                        adjustedConfig.DYE_RESOLUTION = 128;
                        adjustedConfig.SHADING = false;
                    }
    ...
    `;
      ...
      `
                    ...
                    function initFramebuffers() {
                        let simRes = getResolution(adjustedConfig.SIM_RESOLUTION);
                        let dyeRes = getResolution(adjustedConfig.DYE_RESOLUTION);
                    ...
                    function updateColors(dt) {
                        colorUpdateTimer += dt * adjustedConfig.COLOR_UPDATE_SPEED;
                    ...
                    function step(dt) {
                        ...
                        gl.uniform1f(vorticityProgram.uniforms.curl, adjustedConfig.CURL);
                        ...
                        gl.uniform1f(clearProgram.uniforms.value, adjustedConfig.PRESSURE);
                        blit(pressure.write);
                        pressure.swap();
                        ...
                        for (let i = 0; i < adjustedConfig.PRESSURE_ITERATIONS; i++) {
                        ...
                        gl.uniform1f(advectionProgram.uniforms.dissipation, adjustedConfig.VELOCITY_DISSIPATION);
                        blit(velocity.write);
                        velocity.swap();
                        ...
                        gl.uniform1f(advectionProgram.uniforms.dissipation, adjustedConfig.DENSITY_DISSIPATION);
                        blit(dye.write);
                        dye.swap();
                    }
                    ...
                    function splatPointer(pointer) {
                        let dx = pointer.deltaX * adjustedConfig.SPLAT_FORCE;
                        let dy = pointer.deltaY * adjustedConfig.SPLAT_FORCE;
                    ...
                    function splat(x, y, dx, dy, color) {
                        ...
                        gl.uniform1f(splatProgram.uniforms.radius, correctRadius(adjustedConfig.SPLAT_RADIUS / 100.0));
                        blit(velocity.write);
                        velocity.swap();
                    ...
                    // Enhanced color generation with rainbow effects and glow
                    let rainbowHue = 0;
                    let colorTime = 0;
                    ...
                    function generateColor() {
                        if (RAINBOW_MODE) {
                            ...
                            let c = HSVtoRGB(rainbowHue, 0.8, 1.0);
                            ...
                            // Original random colors but enhanced
                            let c = HSVtoRGB(Math.random(), 0.9, 1.0);
                            c.r *= 0.4; // Brighter than original
                            c.g *= 0.4;
                            c.b *= 0.4;
                            return c;
                        }
                    ...
                    // Enhanced color for special effects
                    function generateSpecialColor(intensity = 1.0) {
                        colorTime += 0.02;
                        let hue = (Math.sin(colorTime) * 0.5 + 0.5);
                        let c = HSVtoRGB(hue, 1.0, 1.0);
                        
                        c.r *= intensity * GLOW_INTENSITY;
                        c.g *= intensity * GLOW_INTENSITY;
                        c.b *= intensity * GLOW_INTENSITY;
                    ...
                    // Continuous gentle automatic color bursts every second
                    const autoBurstInterval = setInterval(() => {

                        // Create 8-12 more intense bursts
                        const numBursts = 8 + Math.floor(Math.random() * 5); // 8-12 bursts
                    ...
                    return () => {
                        ...
                        if (window.splashCursorInstance === instanceId) {
                            window.splashCursorActive = false;
                            delete window.splashCursorInstance;
                        }
